package io.wurmatron.plants.common.network;
...
public abstract class CustomMessage <T extends CustomMessage <T>> implements IMessage, IMessageHandler <T, IMessage> {
	...
	public final IMessage onMessage (T msg,MessageContext ctx) {
		if (!msg.isValidOnSide (ctx.side))
			...
			msg.process (PlantsEvolved.proxy.getPlayer (ctx),ctx.side);
		return null;
	}

	private static final <T extends CustomMessage <T>> void checkAndHandle (CustomMessage <T> msg,MessageContext ctx) {
		IThreadListener thread = PlantsEvolved.proxy.getThreadFromCTX (ctx);
		thread.addScheduledTask (() -> msg.process (PlantsEvolved.proxy.getPlayer (ctx),ctx.side));
	}
